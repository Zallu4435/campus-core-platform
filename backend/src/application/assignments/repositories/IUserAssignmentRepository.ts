...
export interface IUserAssignmentRepository {
  getAssignments(subject?: string, status?: string, page?: number, limit?: number, search?: string, studentId?: string, sortBy?: string): Promise<{ assignments: Assignment[]; page: number; limit: number; status?: string; studentId?: string; total: number }>;
